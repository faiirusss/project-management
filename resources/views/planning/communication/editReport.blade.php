...
<div class="col-sm-12 col-xl-10">
    <div class="bg-secondary rounded h-100 p-4">
        <h3 class="mb-4">Edit Reports Data</h3>
        <form action="/reports/{{ $reports->id }}/update" method="post">
            @csrf
        <div class="row mb-3">
            <div class="col-md-12">
                ...
                <select name="name_project" id="nameProject" class="form-select mb-3 text-white" required>
                    ...
                    <option value="{{ $project->id}}" {{ $project->id == $reports->projectDefinition['id'] ? 'selected' : '' }}>{{$project->name_project}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Deliverable</label>
                <input type="text" name="deliverable" value="{{ $reports->deliverable }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Description</label>
                <input type="text" name="description" value="{{ $reports->description }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Delivery Method</label>
                <input type="text" name="delivery_method" value="{{ $reports->delivery_method }}" id="" class="form-control mb-3 text-white" required>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-4">
                <label for="" class="form-label text-white">Frequency</label>
                <input type="text" name="frequency" value="{{ $reports->frequency }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Owner</label>
                <input type="text" name="owner" value="{{ $reports->owner }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Audience</label>
                <input type="text" name="audience" value="{{ $reports->audience }}" id="" class="form-control mb-3 text-white" required>
            </div>
        </div>  
