...
<div class="col-sm-12 col-xl-10">
    <div class="bg-secondary rounded h-100 p-4">
        <h3 class="mb-4">Edit Presentation Data</h3>
        <form action="/projectAnouncement/{{ $projectAnouncement->id }}/update" method="post">
            @csrf
        <div class="row mb-3">
            <div class="col-md-12">
                ...
                <select name="name_project" id="nameProject" class="form-select mb-3 text-white" required>
                    ...
                    <option value="{{ $project->id}}" {{ $project->id == $projectAnouncement->projectDefinition['id'] ? 'selected' : '' }}>{{$project->name_project}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Deliverable</label>
                <input type="text" name="deliverable" value="{{ $projectAnouncement->deliverable }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Description</label>
                <input type="text" name="description" value="{{ $projectAnouncement->description }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Delivery Method</label>
                <input type="text" name="delivery_method" value="{{ $projectAnouncement->delivery_method }}" id="" class="form-control mb-3 text-white" required>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-4">
                <label for="" class="form-label text-white">Frequency</label>
                <input type="text" name="frequency" value="{{ $projectAnouncement->frequency }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Owner</label>
                <input type="text" name="owner" value="{{ $projectAnouncement->owner }}" id="" class="form-control mb-3 text-white" required>
            </div>
            <div class="col-md-4">
                <label for="" class="form-label text-white">Audience</label>
                <input type="text" name="audience" value="{{ $projectAnouncement->audience }}" id="" class="form-control mb-3 text-white" required>
            </div>
        </div>  
